...
class TestMain(unittest.TestCase ):
	...
	def test_choices(self:Self)->None:
		reqt:HttpRequest = self.fact.post('/v1/map-verbs', data={'smpl':"sdf sfsd sfs carry dgdfg dfg dgf" }, )
		...
		reqt = self.fact.post('/v1/map-verbs', data={'smpl':"sdf sfsd sfs dgdfg dfg dgf" }, )
		resp =choices(reqt)
		self.assertEqual(resp.status_code, 200)
		ret= re.search('select name="word[0-9]"', resp.text )
		self.assertEqual(ret, None )

		reqt = self.fact.post('/v1/map-verbs', data={'smpl':"" }, )
		resp =choices(reqt)
		self.assertEqual(resp.status_code, 200)
		ret= re.search('select name="word[0-9]"', resp.text )
		self.assertEqual(ret, None )
