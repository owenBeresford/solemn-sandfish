...
def choices(reqt: HttpRequest) -> HttpResponse:
    ...
    if reqt.method == "GET":
        ...
        fixie:str="You requested an unsupported HTTP verb: "+reqt.method+"\nPlease use GET to access the form, and POST to retun it"
        tmp:HttpResponse=render(reqt, "index.html", {'smpl':fixie, 'choice':None, 'generated':None, 'firsttab':"explore" }, status=406 )
        return tmp
...
def index(reqt: HttpRequest) -> HttpResponse:
    """Inital page impressiion, with not data set.
        IMPURE across Django stack
    """
    tmp:HttpResponse= render(reqt, "index.html", {'smpl':"", 'choice':None, 'generated':None, 'firsttab':"explore" } )
    if reqt.method!="GET" : 
        tmp.status_code=409
    return tmp
